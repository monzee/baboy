...
CSSD_FILES := $(call prefix_replace,$(LESS_FILES:.less=.css.d),$(SRC_LESS),$(TARGET_CSS))
...
JSD_FILES := $(call prefix_replace,$(addsuffix .d,$(ES6_LIB)),$(SRC_ES6),$(TARGET_JS))
...
.PHONY: all help clean clean-all css from js css-deps js-deps which-dist which-target debug
...
js-deps: $(JSD_FILES) $(JSD_BIN_FILES)

$(CSSD_FILES): $(TARGET_CSS)/%.css.d: $(SRC_LESS)/%.less
	# generating dependency list $@
	@mkdir -p $(@D)
	@$(LESSC_DEPS) $< $($(WHICH)_CSS)/$*.css > $@

$(CSS_BIN): $(DIST_CSS)/%.css: $(TARGET_CSS)/%.css.d | which-dist
	# compiling $(SRC_LESS)/$*.less -> $@
	@mkdir -p $(@D)
	@$(LESSC) $(SRC_LESS)/$*.less $@

$(CSS_LIB): $(TARGET_CSS)/%.css: $(TARGET_CSS)/%.css.d | which-target
